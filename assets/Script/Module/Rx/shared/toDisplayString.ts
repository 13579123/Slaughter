...
import { isString , isArray , isObject, isFunction, isMap, isPlainObject, isSet, isSymbol, objectToString } from "./general"
...
const replacer = (_key: string, val: unknown): any => {
  if (isRef(val)) {
    ...
    return {
      [`Map(${val.size})`]: Array.from(val.entries()).reduce(
        (entries, [key, val], i) => {
    ...
    return {
      [`Set(${val.size})`]: Array.from(val.values()).map(v => stringifySymbol(v)),
    }
