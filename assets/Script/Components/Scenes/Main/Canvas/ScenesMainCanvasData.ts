...
export class ScenesMainCanvasData extends ExtensionComponent {
    ...
    protected start() {
        this.effect(() => {
            // 根据本地信息生成角色对象
            const character = Rx.reactive(new CharacterInstance({
                lv: characterManager.data.lv,
                equipments: equipmentManager.data.equipment,
                skills: [{lv: 1 , prototype: getSkillKey(Iatrotechnics)}],
                extraProperty: characterManager.data.currentCharacter.extraProperty,
